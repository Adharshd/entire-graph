...
| `eaab69120ba4` | `8c5afc4` | A defect the response exposed | `--dependency internal/sem` returned zero invalidated on a repository where 139 files depend on it. Zero reads as compliance; it meant the matcher never understood the question — the same shape as reporting an unparsed file SAFE, one level up. |
| `7c63755b0bd0` | `7265afd` | The loop closed with a referee | Acceptance criteria as data, and a verifier with no model in it. Deleting the file drives the invalidated count to zero and the original check certifies it; two invariants catch it. `PARTIAL` is the correct verdict on a real fix, because behaviour preservation is not decidable from a diff. |
| `d8b69a4ce3b5` | `2a9e81a` | The second audience | Seven MCP tools. Three of them answer the person who *receives* the constraint rather than the one who fixes it, and none of them may say SAFE — a green result reads as proof to a reader without the instinct to distrust it. |
| `6bfc359cee62` | `ad03eb5` | Final: the constraint can name its subject | `--from` completes the rule. "X may no longer reach Y" needed both halves; the command could only say Y, which made every architecture rule repo-wide. 139 invalidated becomes 23 when the rule is scoped to the layer it was actually about. |
